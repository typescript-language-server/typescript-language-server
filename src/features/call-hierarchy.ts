...
export function fromProtocolCallHierarchyItem(item: tsp.CallHierarchyItem, documents: LspDocuments): lsp.CallHierarchyItem {
    const useFileName = isSourceFileItem(item);
    const name = useFileName ? path.basename(item.file) : item.name;
    // TODO
    // const detail = useFileName ? vscode.workspace.asRelativePath(path.dirname(item.file)) : item.containerName ?? '';
    const detail = useFileName ? path.dirname(item.file) : item.containerName ?? '';
...
export function fromProtocolCallHierarchyIncomingCall(item: tsp.CallHierarchyIncomingCall, documents: LspDocuments): lsp.CallHierarchyIncomingCall {
    return {
        from: fromProtocolCallHierarchyItem(item.from, documents),
        fromRanges: item.fromSpans.map(Range.fromTextSpan),
    };
}

export function fromProtocolCallHierarchyOutgoingCall(item: tsp.CallHierarchyOutgoingCall, documents: LspDocuments): lsp.CallHierarchyOutgoingCall {
    return {
        to: fromProtocolCallHierarchyItem(item.to, documents),
        fromRanges: item.fromSpans.map(Range.fromTextSpan),
    };
}
