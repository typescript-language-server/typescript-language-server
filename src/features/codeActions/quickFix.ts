...
import { type DiagnosticsManager } from '../../diagnostic-queue.js';
...
type ApplyFixAllCodeAction_args = {
    readonly action: TsQuickFixAllCodeAction;
};
...
class ApplyFixAllCodeAction implements Command {
    ...
    public async execute(args: ApplyFixAllCodeAction_args): Promise<void> {
        if (args.action.combinedResponse) {
            await applyCodeActionCommands(this.client, args.action.combinedResponse.body.commands);
        }
    }
}
...
class CodeActionSet {
    ...
    private readonly _fixAllActions = new Map<{}, TsQuickFixCodeAction>();
    ...
    public addFixAllAction(fixId: {}, action: TsQuickFixCodeAction) {
...
export class TypeScriptQuickFixProvider implements CodeActionProvider<TsQuickFixCodeAction> {
    ...
    private addFixAllForTsCodeAction(
        ...
        // TODO: Recursive property causes error on stringifying.
        // action.command = {
