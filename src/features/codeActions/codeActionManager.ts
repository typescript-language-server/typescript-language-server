...
export class CodeActionManager {
    ...
    // global id => TsCodeAction map need for the resolve request
    // TODO: make it bound
    private tsCodeActionsMap = new Map<number, TsCodeAction>;
        ...
        for (const [providerId, provider] of this.providerMap.entries()) {
            ...
            for (const action of codeActions) {
                ...
                if (provider.isCodeActionResolvable(action)) {
                    ...
                    this.tsCodeActionsMap.set(globalId, action);
                    lspCodeAction.data = {
                        globalId,
                        providerId,
    ...
    public async resolveCodeAction(codeAction: lsp.CodeAction, token?: lsp.CancellationToken): Promise<lsp.CodeAction> {
        ...
        const tsCodeAction = this.tsCodeActionsMap.get(globalId);
