...
import { toSymbolKind, Range } from './protocol-translation.js';
...
export function collectDocumentSymbols(parent: tsp.NavigationTree, symbols: lsp.DocumentSymbol[]): boolean {
    return collectDocumentSymbolsInRange(parent, symbols, { start: typeConverters.Range.fromTextSpan(parent.spans[0]).start, end: typeConverters.Range.fromTextSpan(parent.spans[parent.spans.length - 1]).end });
}
    ...
    for (const span of parent.spans) {
        ...
        if (parent.childItems) {
            for (const child of parent.childItems) {
                if (child.spans.some(childSpan => !!Range.intersection(spanRange, typeConverters.Range.fromTextSpan(childSpan)))) {
        ...
        if (parent.nameSpan) {
            const nameRange = typeConverters.Range.fromTextSpan(parent.nameSpan);
    ...
    for (const span of current.spans) {
        ...
        if (current.childItems) {
            for (const child of current.childItems) {
                if (child.spans.some(span => !!Range.intersection(range, typeConverters.Range.fromTextSpan(span)))) {
