...
import { DiagnosticLanguage } from '../utils/languageDescription.js';
import { ResourceMap } from '../utils/resourceMap.js';
import { Range } from '../utils/typeConverters.js';

function diagnosticsEquals(a: lsp.Diagnostic, b: lsp.Diagnostic): boolean {
    if (a === b) {
        return true;
    }
...
class FileDiagnostics {
    private readonly _diagnostics = new Map<DiagnosticKind, ReadonlyArray<lsp.Diagnostic>>();
    ...
    public getAllDiagnostics(settings: DiagnosticSettings): lsp.Diagnostic[] {
    ...
    public delete(toDelete: lsp.Diagnostic): void {
    ...
    private getSuggestionDiagnostics(settings: DiagnosticSettings) {
        ...
        return this.get(DiagnosticKind.Suggestion).filter(x => {
            if (!enableSuggestions) {
                // Still show unused
                return x.tags && (x.tags.includes(lsp.DiagnosticTag.Unnecessary) || x.tags.includes(lsp.DiagnosticTag.Deprecated));
            }
            return true;
        });
    }

    private get(kind: DiagnosticKind): ReadonlyArray<lsp.Diagnostic> {
...
export class DiagnosticsManager extends Disposable {
    ...
    public setValidate(language: DiagnosticLanguage, value: boolean): void {
    ...
    public setEnableSuggestions(language: DiagnosticLanguage, value: boolean): void {
    ...
    public deleteDiagnostic(resource: Uri, diagnostic: lsp.Diagnostic): void {
    ...
    public getDiagnostics(file: Uri): ReadonlyArray<lsp.Diagnostic> {
        return this._currentDiagnostics.get(file) || [];
    }

    private scheduleDiagnosticsUpdate(file: Uri) {
    ...
    private updateCurrentDiagnostics(file: Uri): void {
        if (this._pendingUpdates.has(file)) {
