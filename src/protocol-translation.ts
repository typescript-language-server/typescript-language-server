...
export function tspLocationToLspPosition(location: tsp.Location): lsp.Position {
    return {
...
export function tspFileSpanToLspLocation(fileSpan: tsp.FileSpan): lsp.Location {
    return {
        uri: pathToUri(fileSpan.file),
        range: {
            start: tspLocationToLspPosition(fileSpan.start),
            end: tspLocationToLspPosition(fileSpan.end)
        }
    };
}
...
export function toDiagnostic(tspDiag: tsp.Diagnostic): lsp.Diagnostic {
    return {
        range: {
            start: tspLocationToLspPosition(tspDiag.start),
            end: tspLocationToLspPosition(tspDiag.end)
        },
